...
public class ServerItemHelper {
    ...
    public static boolean canEquipInSlot(Player player, GameItem item, int slotIdx) {
        ...
        // Strict slot match — legacy ability items (now class-baked into
        // the character) carry targetSlot=-1, which the old `>= 0` guard
        // treated as "any slot". That let players drop a Necrotic Skull
        // into the weapon slot and machine-gun enemies with paralysis.
        // Require an exact match so -1 (no valid slot) is rejected from
        // every slot.
